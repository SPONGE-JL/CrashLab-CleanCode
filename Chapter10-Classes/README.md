...
A. 🙅‍♀️ 아니요! 응집도가 가장 높은 클래스는 가능하지도 바람직하지도 않습니다!
...
[큰 함수 쪼개서 작은 함수/클래스 여럿으로 쪼개는 예제](https://github.com/SPONGE-JL/CrashLab-CleanCode/commit/95eab3e88682cc8dd9e6bd5f443dc033098d98d4)
...
- 책임이 많고 변경이 어려운 기존 버스 클래스: [heavyBus.js](./heavyBus.js)
- 책임을 추상화해서 나눈 작은 버스 클래스
  - 추상 버스: [Bus.ts](./ocp-bus-classes/Bus.ts)
  - 이층 버스: [DoubleDeckBus.ts](./ocp-bus-classes/DoubleDeckBus.ts)
  - 트레일러 버스: [DockingTrailerBus.ts](./ocp-bus-classes/DockingTrailerBus.ts)
  - 테스트 결과: [test.ts](./ocp-bus-classes/test.ts)

    ```json
    { name: '이층버스', capa: 60, spec: { deck: { count: 2 } } }
    { name: '트레일러 버스', capa: 100, spec: { dock: { type: '클로이 후크형' } } }
    ```
